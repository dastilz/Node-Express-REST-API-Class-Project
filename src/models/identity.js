...
const selectSuggestions = async (req) => {
    return await knex.raw(
        "SELECT DISTINCT Identity2.* FROM Identity AS Identity1 " +        
            ...
            "WHERE Identity1.idnum = (SELECT idnum FROM Identity WHERE handle = ? AND password = ?) AND NOT Follows2.followed = (SELECT idnum FROM Identity WHERE handle = ? AND password = ?) " +
                "AND Follows2.followed NOT IN (SELECT Follows3.followed FROM Follows AS Follows3 WHERE Follows3.follower = (SELECT idnum FROM Identity WHERE handle = ? AND password = ?)) " +
                "AND (SELECT idnum FROM Identity WHERE handle = ? AND password = ?) NOT IN (SELECT Block.blocked FROM Block WHERE Block.idnum = Follows2.followed) LIMIT 4",
        [req.handle, req.password, req.handle, req.password, req.handle, req.password, req.handle, req.password]
    )
    .then((data) => data[0])
}
